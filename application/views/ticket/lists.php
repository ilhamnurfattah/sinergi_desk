		...
		<div class="card card-info card-outline">
			<div class="card-header">
				...
				<div class="card-tools">
					<a href="<?= site_url('ticket/create') ?>" class="btn btn-info btn-sm" title="Add"><i class="fa fa-plus-circle"></i> Add</a>
				</div>
			</div>
			<div class="card-body">
				<div class="table-responsive">
					<table class="table table-bordered table-striped datatable" style="font-size: 10pt; padding: 5px">
						...
						<?php foreach($ticket as $key => $row) : ?>
							<tr>
								<td><?= $key+1 ?>.</td>
								<td><a href="<?= site_url('ticket/view/'.$row->id_ticket) ?>"><?= $row->id_ticket ?></a></td>
								<td><?= $row->pelapor ?></td>
								...
								<td style="text-align: center">
									<div class="btn-group btn-group-sm">
										<a href="<?= site_url('ticket/edit/'.$row->id_ticket) ?>" class="btn btn-default" title="Edit"><i class="fa fa-edit"></i></a>
